...
class SinglyLinkedList {
  ...
  {}
    ...
    bool reverse();
    bool removeAll( SLL_Node* );
    bool emptylist( int );

    int menu();
};

bool SinglyLinkedList::updateValue( int index, int updatedValue ) {
  size_t count = 1;
  SLL_Node* marker = head;
...
bool SinglyLinkedList::removeAll(SLL_Node* targetNode) {
...
int SinglyLinkedList::menu() {
  ...
  while(more) {
    ...
    switch(choice) {
      ...
      case 11: {
                 int item;
                 cout<<"\nlast persisting node should be\t";
                 cin>>item;
                 emptylist(item);
               }
               break;
